...
class MacroProcessor {
	...
	static void pass1() throws Exception {
		String s = new String();
		input = new BufferedReader(new InputStreamReader(new FileInputStream("input1.txt")));
		try (PrintWriter output = new PrintWriter(new FileOutputStream("output_pass1.txt"), true)) {
	...
	static void showAla(int pass) throws Exception {
		try (PrintWriter out = new PrintWriter(new FileOutputStream("out_ala_pass" + pass + ".txt"), true)) {
	...
	static void showMnt() throws Exception {
		try (PrintWriter out = new PrintWriter(new FileOutputStream("out_MNT.txt"), true)) {
			for(MntTuple l : mnt) {
				System.out.println(l);
				out.println(l);
			}
	...
	static void showMdt() throws Exception {
		try (PrintWriter out = new PrintWriter(new FileOutputStream("out_MDT.txt"), true)) {
			for(String l : mdt) {
				System.out.println(l);
				out.println(l);
			}
	...
	static void pass2() throws Exception {
		...
		try (PrintWriter output = new PrintWriter(new FileOutputStream("output_pass2.txt"), true)) {
			String token = new String();
			String s;
			while((s = input.readLine()) != null) {
				StringTokenizer st = new StringTokenizer(s, " ", false);
